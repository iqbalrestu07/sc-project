...
func (r *Repository) ListPages(orgID string) ([]Page, error) {
	rows, err := r.db.Query(`
		SELECT DISTINCT ON (page_id) id, page_id, data, created_at, updated_at
		FROM cms_pages
		WHERE (organization_id = $1 OR organization_id IS NULL)
		  AND deleted_at IS NULL
		ORDER BY page_id ASC, (organization_id = $1) DESC, updated_at DESC
	`, orgID)
	if err != nil {
...
func (r *Repository) GetPage(pageID, orgID string) (*Page, error) {
	row := r.db.QueryRow(`
		SELECT id, page_id, data, created_at, updated_at
		FROM cms_pages
		WHERE page_id = $1
		  AND (organization_id = $2 OR organization_id IS NULL)
		  AND deleted_at IS NULL
		ORDER BY (organization_id = $2) DESC, updated_at DESC
		LIMIT 1
	`, pageID, orgID)
